...
To investigate the impact of parameters' modifications on the clustering performance of the scEVE framework, a benchmark can be conducted by calling the function `get_benchmark.sceve()`.
...
-   `time` and `peak_memory` measure the computational performance of the framework. They correspond to the time and the peak memory usage of the algorithm.
...
-   `purity` and `silhouette` are intrinsic measurements of the clustering performance of the framework. They leverage the gene expression of the cells. Their value range from 0 to 1, and it is proportional to the intra-cluster homogeneity and the inter-cluster dissimilarity. They are calculated with the package `bluster` [@lun2024].

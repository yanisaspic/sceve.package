...
```{r dataset}
Darmanis_HumGBM <- load_data("Darmanis_HumGBM")
...
The loaded `Darmanis_HumGBM` data include the scRNA-seq expression matrix `expression.init`, and the cell types identified by Darmanis *et al.* `ground_truth`.
...
The results of the `sceve()` function is a list with two elements: `records` and `labels`.
...
```{r labels}
set.seed(1)
sample(results$labels, size=10)
```

`labels` corresponds to the most informative cluster labels (*i.e.* the ones with the highest resolution) predicted by scEVE for every cell in the dataset. Basically, the type of output you would get from using any clustering method.
...
To investigate the impact of parameters' modifications on the clustering performance of the scEVE framework, a benchmark can be conducted by calling the function `get_benchmark_sceve()`.

```{r get_benchmark.sceve}
params <- get_default_parameters()
# get_benchmark_sceve(expression.init, params)
```

This benchmark returns six different metrics :

-   `time` and `peak_memory_usage` measure the computational performance of the framework. They correspond to the time and the peak memory usage of the algorithm.
-   `ARI` and `NMI` are extrinsic measurements of the clustering performance of the framework. They leverage the ground truth predicted by the authors of the datasets. Their value range from 0 to 1, and it is proportional to the similarity between the clusters predicted by the framework and the ground truth. They are calculated with the package `aricode` [@chiquet2024].
-   `neighbor_purity` and `silhouette` are intrinsic measurements of the clustering performance of the framework. They leverage the gene expression of the cells. Their value range from 0 to 1, and it is proportional to the intra-cluster homogeneity and the inter-cluster dissimilarity. They are calculated with the package `bluster` [@lun2024].

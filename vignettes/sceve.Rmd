...
The composite U-MAP plots drawn with `SCpubr` [@blanco-carmona2022] show the meta-clusters predicted, as well as their respective robustness. For instance, `C.1` has a robustness of 0.49, and its cells are projected in the bottom-left red blob. **Note** that some robust clusters might be missing in the combined plot due to color overlaps.
...
After identifying marker genes in each meta-cluster, scEVE attempts to identify **characterized clusters**. By default, characterized clusters are defined as meta-clusters with 5 specific marker genes, at least. Robust clusters insufficiently characterized are merged to the leftover cluster, so as to prevent over-clustering. **Note** that a second attempt at characterizing the leftover cluster is conducted after the merging, so as to account for the new cells present in the cluster.\
...
print(length(characterized_clusters$C.3$specific_markers))
```

Here, we can see every marker gene predicted for the meta-cluster `C.3`, as well as their log2 fold change. We can also see that the cluster `C.3` has 62 specific marker genes.
...
The resulting upset-plot drawn with ggVennDiagram [@gao2021; @gao2024] is divided into a left and a right component. On the left, a bar-plot indicates the number of marker genes predicted in each cluster. By exploiting the bar-plot, we see that `C.1` has 90 marker genes, whereas the leftover cluster `C.L` has more than 140 marker genes. On the right, a bar-plot and a grid-plot report the distribution of marker genes across clusters. For instance, the first bar of the plot is associated to a single `C.1` dot on the grid. By leveraging both plots, we can learn that the cluster `C.1` has 62 specific marker genes. The fifth bar, which is associated to `C.1` and `C.2` on the grid, inform us that the two clusters share 0 common marker genes.
...
By default, it is set to `get_characterized_clusters.specific_markers_threshold`, and a specific marker genes threshold is used to identify characterized clusters, with threshold=5. If at least two biologically distinct clusters are characterized, they are returned.
